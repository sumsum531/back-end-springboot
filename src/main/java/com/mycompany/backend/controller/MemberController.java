...
//    String accessToken = Jwt.createAccessToken(member.getMid(), "ROLE_USER");
//    String refreshToken = Jwt.createRefreshToken(member.getMid(), "ROLE_USER");
  ...
  public ResponseEntity<String> refreshToken(
    ...
    //동일한 토큰인지 확인
    ValueOperations<String, String> vo = redisTemplate.opsForValue();
    String redisRefreshToken = vo.get(accessToken); //얘가 null이냐 아니냐!
    //null이면 accessToken이 없는 토큰을 검색한 것!() null이 아니면 accessToken이 맞는것을 의미
    if(redisRefreshToken == null) {
      return ResponseEntity.status(401).body("wrong access token");
    }
    
    //이게 다르면 위조가 되었다는 것임!
    if(!refreshToken.equals(redisRefreshToken)) {//따라서 다를경우
    ...
    //Redis에 새로운 정보를 삭제함
    //refresh토큰의 경우, access토큰만 새로 신청하기 때문에, 굳이 refreshToken의 duration을 넣을 필요없음
    vo.set(accessToken, refreshToken, Jwt.REFRESH_TOKEN_DURATION, TimeUnit.MILLISECONDS);
  ...
  public ResponseEntity<String> logout(@RequestHeader("Authorization") String authorization){
    //Access Token을 얻기
    String accessToken = Jwt.getAccessToken(authorization);
    //사용하기 위해선 유효해야 하기 때문에!
    //즉, accessToken이 null이거나 유효하지 않으면 오류 메세지! 
    if(accessToken == null) { 
    ...
    //RefreshToken 쿠키 삭제(클라이언트에 저장된 쿠키와 같은 이름의 쿠키를 만드는데 maxAge를 0으로 강제 세팅하고 이 쿠키를 다시 set해주면 삭제된다.)
